...
resource "proxmox_vm_qemu" "vm" {
 ...
 ipconfig1    = var.vm_ipconfig[count.index]
 tags         = var.vm_tags
 boot         = var.vm_boot_device
 bios         = var.vm_bios
 agent        = var.vm_qemu_agent
 force_create = true
 os_type      = var.vm_ostype
...
resource "null_resource" "shell" {
  ...
  provisioner "local-exec" {
    command = "ANSIBLE_CONFIG=../../../ansible/playbooks/linux/ansible.cfg ansible-playbook ../../../ansible/playbooks/linux/deploy-homelab.yml"
  }
}
